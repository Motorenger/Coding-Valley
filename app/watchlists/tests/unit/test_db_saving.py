...
def test_save_movie(mocker):
    ...
    # when
    result = save_movie(omdb_id)
    # then
    assert Movie.objects.count() == 1, "The db must have one movie"
    assert result.title == movie_data.get('Title'), "The title of test data is not equal to the title of movie data"
    assert result.genres == movie_data.get('Genre'), "The genres of test data is not equal to the genres of movie data"
...
def test_save_series(mocker):
    ...
    # when
    result = save_series(omdb_id)
    # then
    assert Series.objects.count() == 1, "The db must have one series"
    assert result.title == series_data.get('Title'), "The title of test data is not equal to the title of movie data"
    assert result.genres == series_data.get('Genre'), "The genres of test data is not equal to the genres of movie data"
